...
echo "Minutes: " . $droid->getFillMinutes();
...
class RepairDroid
{
            ...
            foreach ($this->getAdjacentCoordinates() as $moveDirection => $targetPosition) {
                ...
                switch ($status) {
                    ...
                    case Room::TILE_OXYGEN:
                        $this->position = $targetPosition;
                        $this->oxygenPosition = $targetPosition;
                }
            }
    ...
    /**
     * This A* makes more sense
     */
    public function getFillMinutes(): int
    {
        $minutes = 0;
        $points = [$this->oxygenPosition];
        $weights = [];
        while (++$minutes) {
            $endPoints = [];
            foreach ($points as $point) {
                foreach ($this->getAdjacentCoordinates($point) as $target) {
                    if ($this->room->canMoveOn($target) && !isset($weights[$target->y][$target->x])) {
                        $weights[$target->y][$target->x] = $minutes;
                        $endPoints[] = $target;
                    }
                }
            }
